...
public abstract class SeekableStreamIndexTaskRunner<PartitionIdType, SequenceOffsetType> implements ChatHandler {
  public enum Status {
    NOT_STARTED, STARTING, READING, PAUSED, PUBLISHING
  }
  ...
  // currOffsets are what should become the start offsets of the next reader, if
  // we stopped reading now. They are
  ...
  // The pause lock and associated conditions are to support coordination between
  // the Jetty threads and the main
  // ingestion loop. The goal is to provide callers of the API a guarantee that if
  // pause() returns successfully
  // the ingestion loop has been stopped at the returned sequences and will not
  // ingest any more data until resumed. The
  // fields are used as follows (every step requires acquiring [pauseLock]):
  // Pausing:
  // - In pause(), [pauseRequested] is set to true and then execution waits for
  // [status] to change to PAUSED, with the
  // condition checked when [hasPaused] is signalled.
  // - In possiblyPause() called from the main loop, if [pauseRequested] is true,
  // [status] is set to PAUSED,
  // [hasPaused] is signalled, and execution pauses until [pauseRequested] becomes
  // false, either by being set or by
  // the [pauseMillis] timeout elapsing. [pauseRequested] is checked when
  // [shouldResume] is signalled.
  // Resuming:
  // - In resume(), [pauseRequested] is set to false, [shouldResume] is signalled,
  // and execution waits for [status] to
  // change to something other than PAUSED, with the condition checked when
  // [shouldResume] is signalled.
  // - In possiblyPause(), when [shouldResume] is signalled, if [pauseRequested]
  // has become false the pause loop ends,
  ...
  // [statusLock] is used to synchronize the Jetty thread calling stopGracefully()
  // with the main run thread. It prevents
  // the main run thread from switching into a publishing state while the
  // stopGracefully() thread thinks it's still in
  // a pre-publishing state. This is important because stopGracefully() will try
  // to use the [stopRequested] flag to stop
  // the main thread where possible, but this flag is not honored once publishing
  // has begun so in this case we must
  // interrupt the thread. The lock ensures that if the run thread is about to
  // transition into publishing state, it
  // blocks until after stopGracefully() has set [stopRequested] and then does a
  // final check on [stopRequested] before
  ...
  public SeekableStreamIndexTaskRunner(final SeekableStreamIndexTask<PartitionIdType, SequenceOffsetType> task,
      final InputRowParser<ByteBuffer> parser, final AuthorizerMapper authorizerMapper,
      final Optional<ChatHandlerProvider> chatHandlerProvider, final CircularBuffer<Throwable> savedParseExceptions,
  ...
  public TaskStatus run(TaskToolbox toolbox) {
    try {
      ...
      return TaskStatus.failure(task.getId(), errorMsg);
    }
  }
  ...
  public void initializeSequences() throws IOException {
    if (!restoreSequences()) {
      final TreeMap<Integer, Map<PartitionIdType, SequenceOffsetType>> checkpoints = getCheckPointsFromContext(toolbox,
      ...
      if (checkpoints != null) {
        ...
        while (sequenceOffsets.hasNext()) {
          ...
          final Set<PartitionIdType> exclusiveStartPartitions = computeExclusiveStartPartitionsForSequence(
              previous.getValue());
          addSequence(new SequenceMetadata<>(previous.getKey(),
              StringUtils.format("%s_%s", ioConfig.getBaseSequenceName(), previous.getKey()), previous.getValue(),
              current.getValue(), true, exclusiveStartPartitions));
          previous = current;
        }
        final Set<PartitionIdType> exclusiveStartPartitions = computeExclusiveStartPartitionsForSequence(
            previous.getValue());
        addSequence(new SequenceMetadata<>(previous.getKey(),
            StringUtils.format("%s_%s", ioConfig.getBaseSequenceName(), previous.getKey()), previous.getValue(),
            endOffsets, false, exclusiveStartPartitions));
      } else {
        addSequence(new SequenceMetadata<>(0, StringUtils.format("%s_%s", ioConfig.getBaseSequenceName(), 0),
            ioConfig.getStartSequenceNumbers().getPartitionSequenceNumberMap(), endOffsets, false,
  ...
  private TaskStatus runInternal(TaskToolbox toolbox) throws Exception {
    ...
    final LookupNodeService lookupNodeService = lookupTier == null ? toolbox.getLookupNodeService()
        : new LookupNodeService(lookupTier);

    final DiscoveryDruidNode discoveryDruidNode = new DiscoveryDruidNode(toolbox.getDruidNode(), NodeType.PEON,
    ...
    try (final RecordSupplier<PartitionIdType, SequenceOffsetType> recordSupplier = task.newTaskRecordSupplier()) {
      ...
      if (restoredMetadata == null) {
        ...
        Preconditions.checkState(
            sequences.get(0).startOffsets.entrySet().stream()
                .allMatch(partitionOffsetEntry -> createSequenceNumber(partitionOffsetEntry.getValue())
                    .compareTo(createSequenceNumber(ioConfig.getStartSequenceNumbers().getPartitionSequenceNumberMap()
        ...
        final SeekableStreamStartSequenceNumbers<PartitionIdType, SequenceOffsetType> restoredNextPartitions = deserializePartitionsFromMetadata(
            toolbox.getObjectMapper(), restoredMetadataMap.get(METADATA_NEXT_PARTITIONS));
        ...
        if (!restoredNextPartitions.getStream().equals(ioConfig.getStartSequenceNumbers().getStream())) {
          throw new ISE("WTF?! Restored stream[%s] but expected stream[%s]", restoredNextPartitions.getStream(),
        ...
        if (!currOffsets.keySet().equals(ioConfig.getStartSequenceNumbers().getPartitionSequenceNumberMap().keySet())) {
          throw new ISE("WTF?! Restored partitions[%s] but expected partitions[%s]", currOffsets.keySet(),
              ioConfig.getStartSequenceNumbers().getPartitionSequenceNumberMap().keySet());
        }
        // sequences size can be 0 only when all sequences got published and task
      ...
      // Filter out partitions with END_OF_SHARD markers since these partitions have
      // already been fully read. This
      ...
      if (currOffsets.entrySet().removeIf(x -> isEndOfShard(x.getValue()))) {
        log.info("Removed [%d] partitions from assignment which have already been closed",
            numPreFilterPartitions - currOffsets.size());
      }

      // Initialize lastReadOffsets immediately after restoring currOffsets. This is
      // only done when end offsets are
      // inclusive, because the point of initializing lastReadOffsets here is so we
      // know when to skip the start record.
      ...
      if (!isEndOffsetExclusive()) {
        for (Map.Entry<PartitionIdType, SequenceOffsetType> entry : currOffsets.entrySet()) {
          final boolean isAtStart = entry.getValue()
              .equals(ioConfig.getStartSequenceNumbers().getPartitionSequenceNumberMap().get(entry.getKey()));
      ...
      final Supplier<Committer> committerSupplier = () -> {
        ...
        return new Committer() {
          ...
          public Object getMetadata() {
            return ImmutableMap.of(METADATA_NEXT_PARTITIONS, new SeekableStreamStartSequenceNumbers<>(stream, snapshot, Collections.emptySet()));
          }
      ...
      // Main loop.
      // Could eventually support leader/follower mode (for keeping replicas more in
      // sync)
      ...
      try {
        while (stillReading) {
          if (possiblyPause()) {
            // The partition assignments may have changed while paused by a call to
            // setEndOffsets() so reassign
            // partitions upon resuming. Don't call "seekToStartingSequence" after
            // "assignPartitions", because there's
          ...
          // if stop is requested or task's end sequence is set by call to setEndOffsets
          // method with finish set to true
          ...
          // calling getRecord() ensures that exceptions specific to kafka/kinesis like
          // OffsetOutOfRangeException
          // are handled in the subclasses.
          List<OrderedPartitionableRecord<PartitionIdType, SequenceOffsetType>> records = getRecords(recordSupplier,
              toolbox);
          ...
          for (OrderedPartitionableRecord<PartitionIdType, SequenceOffsetType> record : records) {
            ...
            log.trace("Got stream[%s] partition[%s] sequenceNumber[%s], shouldProcess[%s].", record.getStream(),
                record.getPartitionId(), record.getSequenceNumber(), shouldProcess);
            ...
            if (shouldProcess) {
              try {
                ...
                final SequenceMetadata<PartitionIdType, SequenceOffsetType> sequenceToUse = sequences.stream()
                    .filter(sequenceMetadata -> sequenceMetadata.canHandle(this, record)).findFirst().orElse(null);

                if (sequenceToUse == null) {
                  throw new ISE(
                      "WTH?! cannot find any valid sequence for record with partition [%s] and sequenceNumber [%s]. Current sequences: %s",
                      record.getPartitionId(), record.getSequenceNumber(), sequences);
                }

                for (InputRow row : rows) {
                  if (row != null && task.withinMinMaxRecordTime(row)) {
                    final AppenderatorDriverAddResult addResult = driver.add(row, sequenceToUse.getSequenceName(),
                        committerSupplier, true,
                        // do not allow incremental persists to happen until all the rows from this
                        // batch
                        // of rows are indexed
                        false);

                    if (addResult.isOk()) {
                      // If the number of rows in the segment exceeds the threshold after adding a
                      // row,
                      // move the segment out from the active segments of BaseAppenderatorDriver to
                      // make a new segment.
                      ...
                      // If we allow continuing, then consider blacklisting the interval for a while
                      // to avoid constant checks.
              ...
              // in kafka, we can easily get the next offset by adding 1, but for kinesis,
              // there's no way
              // to get the next sequence number without having to make an expensive api call.
              // So the behavior
              // here for kafka is to +1 while for kinesis we simply save the current sequence
              // number
            ...
            // Use record.getSequenceNumber() in the moreToRead check, since currOffsets
            // might not have been
            // updated if we were skipping records for being beyond the end.
            final boolean moreToReadAfterThisRecord = isMoreToReadAfterReadingRecord(record.getSequenceNumber(),
          ...
          if (sequenceToCheckpoint != null && stillReading) {
            Preconditions.checkArgument(
                getLastSequenceMetadata().getSequenceName().equals(sequenceToCheckpoint.getSequenceName()),
                "Cannot checkpoint a sequence [%s] which is not the latest one, sequences %s", sequenceToCheckpoint,
                sequences);
            requestPause();
            final CheckPointDataSourceMetadataAction checkpointAction = new CheckPointDataSourceMetadataAction(
                task.getDataSource(), ioConfig.getTaskGroupId(), task.getIOConfig().getBaseSequenceName(),
                createDataSourceMetadata(new SeekableStreamStartSequenceNumbers<>(stream,
                    sequenceToCheckpoint.getStartOffsets(), sequenceToCheckpoint.getExclusiveStartPartitions())),
                createDataSourceMetadata(new SeekableStreamStartSequenceNumbers<>(stream, currOffsets, Collections.emptySet())));
      ...
      for (int i = 0; i < sequences.size(); i++) {
        ...
        if (!publishingSequences.contains(sequenceMetadata.getSequenceName())) {
          ...
          // Update assignments of the sequence, which should clear them. (This will be
          // checked later, when the
          // Committer is built.)
      ...
      // handoffFuture. handoffFuture can throw an exception if 1) the corresponding
      // publishFuture failed or 2) it
      // failed to persist sequences. It might also return null if handoff failed, but
      // was recoverable.
      ...
      if (tuningConfig.getHandoffConditionTimeout() == 0) {
        ...
        try {
          handedOffList = Futures.allAsList(handOffWaitList).get(tuningConfig.getHandoffConditionTimeout(),
              TimeUnit.MILLISECONDS);
        } catch (TimeoutException e) {
          // Handoff timeout is not an indexing failure, but coordination failure. We
          // simply ignore timeout exception
          // here.
      ...
      // (2) catch InterruptedException and RejectedExecutionException thrown for the
      // whole ingestion steps including
      ...
      // if we were interrupted because we were asked to stop, handle the exception
      // and return success, else rethrow
      if (!stopRequested.get()) {
      ...
      // (3) catch all other exceptions thrown for the whole ingestion steps including
      // the final publishing.
      caughtExceptionOuter = e;
      try {
        Futures.allAsList(publishWaitList).cancel(true);
  ...
  private void checkPublishAndHandoffFailure() throws ExecutionException, InterruptedException {
    ...
    final List<ListenableFuture<SegmentsAndMetadata>> publishFinished = publishWaitList.stream().filter(Future::isDone)
        .collect(Collectors.toList());

    for (ListenableFuture<SegmentsAndMetadata> publishFuture : publishFinished) {
      // If publishFuture failed, the below line will throw an exception and catched
      // by (1), and then (2) or (3).
      publishFuture.get();
    }
    ...
    final List<ListenableFuture<SegmentsAndMetadata>> handoffFinished = handOffWaitList.stream().filter(Future::isDone)
        .collect(Collectors.toList());

    for (ListenableFuture<SegmentsAndMetadata> handoffFuture : handoffFinished) {
      // If handoffFuture failed, the below line will throw an exception and catched
      // by (1), and then (2) or (3).
      handoffFuture.get();
    }
  ...
  private void publishAndRegisterHandoff(SequenceMetadata<PartitionIdType, SequenceOffsetType> sequenceMetadata) {
    ...
    final ListenableFuture<SegmentsAndMetadata> publishFuture = Futures.transform(
        driver.publish(sequenceMetadata.createPublisher(this, toolbox, ioConfig.isUseTransaction()),
    ...
    // Create a handoffFuture for every publishFuture. The created handoffFuture
    // must fail if publishFuture fails.
    ...
    Futures.addCallback(publishFuture, new FutureCallback<SegmentsAndMetadata>() {
      ...
      public void onSuccess(SegmentsAndMetadata publishedSegmentsAndMetadata) {
        log.info("Published segments %s with metadata[%s].",
        ...
        Futures.transform(driver.registerHandoff(publishedSegmentsAndMetadata),
            new Function<SegmentsAndMetadata, Void>() {
              ...
              public Void apply(@Nullable SegmentsAndMetadata handoffSegmentsAndMetadata) {
                if (handoffSegmentsAndMetadata == null) {
                  log.warn("Failed to handoff segments %s",
                      Lists.transform(publishedSegmentsAndMetadata.getSegments(), DataSegment::getId));
                }
    ...
    if (sequencesPersistFile.exists()) {
      sequences = new CopyOnWriteArrayList<>(
          toolbox.getObjectMapper().<List<SequenceMetadata<PartitionIdType, SequenceOffsetType>>>readValue(
              sequencesPersistFile, getSequenceMetadataTypeReference()));
      return true;
    } else {
      return false;
    }
  ...
  private synchronized void persistSequences() throws IOException {
    ...
    toolbox.getObjectMapper().writerWithType(getSequenceMetadataTypeReference())
  ...
  private Map<String, TaskReport> getTaskCompletionReports(@Nullable String errorMsg) {
    return TaskReport.buildTaskReports(
        new IngestionStatsAndErrorsTaskReport(task.getId(), new IngestionStatsAndErrorsTaskReportData(ingestionState,
            getTaskCompletionUnparseableEvents(), getTaskCompletionRowStats(), errorMsg)));
  }
  ...
  private Map<String, Object> getTaskCompletionRowStats() {
    ...
    metrics.put(RowIngestionMeters.BUILD_SEGMENTS, rowIngestionMeters.getTotals());
    return metrics;
  }
    ...
    for (SequenceMetadata<PartitionIdType, SequenceOffsetType> sequenceMetadata : sequences) {
      ...
      if (!sequenceMetadata.isOpen() && !publishingSequences.contains(sequenceMetadata.getSequenceName())) {
        ...
        try {
          ...
          log.info("Persist completed with results: [%s], adding sequence [%s] to publish queue", result,
              sequenceMetadata);
          publishAndRegisterHandoff(sequenceMetadata);
  ...
  private Set<StreamPartition<PartitionIdType>> assignPartitions(
    ...
    for (Map.Entry<PartitionIdType, SequenceOffsetType> entry : currOffsets.entrySet()) {
      ...
      if (!isRecordAlreadyRead(partition, endOffset) && isMoreToReadBeforeReadingRecord(currOffset, endOffset)) {
        log.info("Adding partition[%s], start[%s] -> end[%s] to assignment.", partition, currOffset, endOffset);
  ...
  private void addSequence(final SequenceMetadata<PartitionIdType, SequenceOffsetType> sequenceMetadata) {
    // Sanity check that the start of the new sequence matches up with the end of
    // the prior sequence.
    for (Map.Entry<PartitionIdType, SequenceOffsetType> entry : sequenceMetadata.getStartOffsets().entrySet()) {
      ...
      if (!sequences.isEmpty()) {
        ...
        if (!startOffset.equals(priorOffset)) {
          throw new ISE("New sequence startOffset[%s] does not equal expected prior offset[%s]", startOffset,
              priorOffset);
        }
      }
    }
    ...
    if (!isEndOffsetExclusive() && !sequences.isEmpty()) {
      ...
      if (!lastMetadata.endOffsets.keySet().equals(sequenceMetadata.getExclusiveStartPartitions())) {
        throw new ISE("Exclusive start partitions[%s] for new sequence don't match to the prior offset[%s]",
            sequenceMetadata.getExclusiveStartPartitions(), lastMetadata);
      }
    }
   ...
   * lastReadOffsets.
   */
  private boolean isRecordAlreadyRead(final PartitionIdType recordPartition,
   ...
   * recordSequenceNumber and end at endSequenceNumber, there is more left to
   * read. Used in pre-read checks to determine if there is anything left to read.
   */
  private boolean isMoreToReadBeforeReadingRecord(final SequenceOffsetType recordSequenceNumber,
  ...
  /**
   * Returns true if, given that recordSequenceNumber has already been read and we
   * want to end at endSequenceNumber, there is more left to read. Used in
   * post-read checks to determine if there is anything left to read.
   */
  private boolean isMoreToReadAfterReadingRecord(final SequenceOffsetType recordSequenceNumber,
      final SequenceOffsetType endSequenceNumber) {
    final int compareNextToEnd = createSequenceNumber(getNextStartOffset(recordSequenceNumber))
        .compareTo(createSequenceNumber(endSequenceNumber));

    // Unlike isMoreToReadBeforeReadingRecord, we don't care if the end is exclusive
    // or not. If we read it, we're done.
    return compareNextToEnd < 0;
  }

  private void seekToStartingSequence(RecordSupplier<PartitionIdType, SequenceOffsetType> recordSupplier,
      Set<StreamPartition<PartitionIdType>> partitions) throws InterruptedException {
  ...
  /**
   * Checks if the pauseRequested flag was set and if so blocks: a) if pauseMillis
   * == PAUSE_FOREVER, until pauseRequested is cleared b) if pauseMillis !=
   * PAUSE_FOREVER, until pauseMillis elapses -or- pauseRequested is cleared
   * <p>
   * If pauseMillis is changed while paused, the new pause timeout will be
   * applied. This allows adjustment of the pause timeout (making a timed pause
   * into an indefinite pause and vice versa is valid) without having to resume
   * and ensures that the loop continues to stay paused without ingesting any new
   * events. You will need to signal shouldResume after adjusting pauseMillis for
  ...
  private void handleParseException(ParseException pe, OrderedPartitionableRecord record) {
    ...
    if (rowIngestionMeters.getUnparseable() + rowIngestionMeters.getProcessedWithError() > tuningConfig
  ...
  protected void sendResetRequestAndWait(Map<StreamPartition<PartitionIdType>, SequenceOffsetType> outOfRangePartitions,
      TaskToolbox taskToolbox) throws IOException {
    Map<PartitionIdType, SequenceOffsetType> partitionOffsetMap = outOfRangePartitions.entrySet().stream()
        .collect(Collectors.toMap(x -> x.getKey().getPartitionId(), Map.Entry::getValue));

    boolean result = taskToolbox.getTaskActionClient()
        .submit(new ResetDataSourceMetadataAction(task.getDataSource(),
            createDataSourceMetadata(new SeekableStreamStartSequenceNumbers<>(
                ioConfig.getStartSequenceNumbers().getStream(), partitionOffsetMap,
    ...
    if (result) {
      log.makeAlert("Resetting sequences for datasource [%s]", task.getDataSource())
          .addData("partitions", partitionOffsetMap.keySet()).emit();
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public Response setEndOffsetsHTTP(Map<PartitionIdType, SequenceOffsetType> sequences,
      ...
      @Context final HttpServletRequest req) throws InterruptedException {
    authorizationCheck(req, Action.WRITE);
  ...
  @VisibleForTesting
  public Response setEndOffsets(Map<PartitionIdType, SequenceOffsetType> sequenceNumbers, boolean finish // this field
                                                                                                         // is only for
                                                                                                         // internal
                                                                                                         // purposes,
                                                                                                         // shouldn't be
    ...
    if (sequenceNumbers == null) {
      return Response.status(Response.Status.BAD_REQUEST)
          .entity("Request body must contain a map of { partition:endOffset }").build();
    } else if (!endOffsets.keySet().containsAll(sequenceNumbers.keySet())) {
      return Response.status(Response.Status.BAD_REQUEST).entity(StringUtils
          .format("Request contains partitions not being handled by this task, my partitions: %s", endOffsets.keySet()))
          .build();
    } else {
      try {
        pauseLock.lockInterruptibly();
        ...
        if (isEndOffsetExclusive()) {
          // When end offsets are exclusive, there's no need for marking the next sequence
          // as having any
          // exclusive-start partitions. It should always start from the end offsets of
          // the prior sequence.
        ...
        if ((latestSequence.getStartOffsets().equals(sequenceNumbers)
            && latestSequence.getExclusiveStartPartitions().equals(exclusiveStartPartitions) && !finish)
          ...
          return Response.status(Response.Status.BAD_REQUEST)
              .entity(StringUtils.format("WTH?! Sequence [%s] has already endOffsets set, cannot set to [%s]",
                  latestSequence, sequenceNumbers))
              .build();
        } else if (!isPaused()) {
          return Response.status(Response.Status.BAD_REQUEST)
              .entity("Task must be paused before changing the end sequences").build();
        }

        for (Map.Entry<PartitionIdType, SequenceOffsetType> entry : sequenceNumbers.entrySet()) {
          if (createSequenceNumber(entry.getValue())
              .compareTo(createSequenceNumber(currOffsets.get(entry.getKey()))) < 0) {
            return Response.status(Response.Status.BAD_REQUEST)
                .entity(StringUtils.format("End sequence must be >= current sequence for partition [%s] (current: %s)",
                    entry.getKey(), currOffsets.get(entry.getKey())))
                .build();
          }
        }
        ...
        if (finish) {
          ...
          final SequenceMetadata<PartitionIdType, SequenceOffsetType> newSequence = new SequenceMetadata<>(
              ...
              sequenceNumbers, endOffsets, false, exclusiveStartPartitions);
        ...
        resume();
        return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(Throwables.getStackTraceAsString(e))
            .build();
      } finally {
        pauseLock.unlock();
      }
  ...
  private Map<Integer, Map<PartitionIdType, SequenceOffsetType>> getCheckpoints() {
    return new TreeMap<>(sequences.stream()
        .collect(Collectors.toMap(SequenceMetadata::getSequenceId, SequenceMetadata::getStartOffsets)));
  }
   ...
   *         started publishing; 202 Accepted if the method has timed out and
   *         returned before the task has paused; 200 OK with a map of the current
   *         partition sequences in the response body if the task successfully
   *         paused
   */
  @POST
  @Path("/pause")
  @Produces(MediaType.APPLICATION_JSON)
  public Response pauseHTTP(@Context final HttpServletRequest req) throws InterruptedException {
    authorizationCheck(req, Action.WRITE);
    return pause();
  }
  ...
  public Response pause() throws InterruptedException {
    if (!(status == Status.PAUSED || status == Status.READING)) {
      return Response.status(Response.Status.BAD_REQUEST)
          .entity(StringUtils.format("Can't pause, task is not in a pausable state (state: [%s])", status)).build();
    }
    ...
    try {
      ...
      while (!isPaused()) {
        if (nanos <= 0L) {
          return Response.status(Response.Status.ACCEPTED).entity("Request accepted but task has not yet paused")
              .build();
        }
   ...
   *
   * 1) Verifies that the sequence numbers we read are at least as high as those
   * read previously, and throws an exception if not. 2) Returns false if we
   * should skip this record because it's either (a) the first record in a
   * partition that we are needing to be exclusive on; (b) too late to read, past
   * the endOffsets.
   */
  private boolean verifyRecordInRange(final PartitionIdType partition, final SequenceOffsetType recordOffset) {
    // Verify that the record is at least as high as its currOffset.
    final SequenceOffsetType currOffset = Preconditions.checkNotNull(currOffsets.get(partition),
        "Current offset is null for sequenceNumber[%s] and partition[%s]", recordOffset, partition);
    ...
    if (comparisonToCurrent < 0) {
      throw new ISE("Record sequenceNumber[%s] is smaller than current sequenceNumber[%s] for partition[%s]",
          recordOffset, currOffset, partition);
    }
  ...
  protected abstract TreeMap<Integer, Map<PartitionIdType, SequenceOffsetType>> getCheckPointsFromContext(
      TaskToolbox toolbox, String checkpointsString) throws IOException;

  /**
   * Calculates the sequence number used to update currOffsets after finished
   * reading a record. This is what would become the start offsets of the next
   * reader, if we stopped reading now.
   *
  ...
  /**
   * polls the next set of records from the recordSupplier, the main purpose of
   * having a separate method here is to catch and handle exceptions specific to
   * Kafka/Kinesis
   *
  ...
  protected abstract List<OrderedPartitionableRecord<PartitionIdType, SequenceOffsetType>> getRecords(
      RecordSupplier<PartitionIdType, SequenceOffsetType> recordSupplier, TaskToolbox toolbox) throws Exception;
  ...
  /**
   * create a specific implementation of Kafka/Kinesis sequence number/offset used
   * for comparison mostly
   *
  ...
  /**
   * check if the sequence offsets stored in currOffsets are still valid sequence
   * offsets compared to earliest sequence offsets fetched from stream
   */
  protected abstract void possiblyResetDataSourceMetadata(TaskToolbox toolbox,
      RecordSupplier<PartitionIdType, SequenceOffsetType> recordSupplier,
      Set<StreamPartition<PartitionIdType>> assignment);

  /**
   * In Kafka, the endOffsets are exclusive, so skip it. In Kinesis the endOffsets
   * are inclusive
   */
